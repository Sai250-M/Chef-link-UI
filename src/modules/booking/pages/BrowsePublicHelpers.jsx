...
import { getPublicHelpers } from "../services/helper.service";
import { PublicHelperCard } from "../components/PublicHelperCard";
...
export const BrowsePublicHelpers = () => {
  ...
  const fetchHelpers = useCallback(async () => {
    ...
    try {
      ...
      const params = {
        ...
        ...(rateRange[1] < 5000 && { max_rate: rateRange[1] }),
      };
      const res = await getPublicHelpers(params);
        ...
        <Container maxWidth="lg" sx={{ position: "relative", zIndex: 1 }}>
          <motion.div {...fadeUp(0)}>
            ...
            <Typography variant="h2" fontWeight={800} sx={{ color: "white", mb: 1.5 }}>
              Find a Kitchen Helper
            </Typography>
            <Typography variant="body1" sx={{ color: "rgba(255,255,255,0.6)", mb: 4, maxWidth: 480 }}>
              Browse skilled kitchen helpers and support staff. Book directly — no account needed.
            </Typography>
          </motion.div>
                ...
                <Stack spacing={2.5}>
                  ...
                  <Box>
                    <Typography variant="body2" fontWeight={600} mb={1.5}>
                      Hourly Rate: ₹{rateRange[0].toLocaleString()} – ₹{rateRange[1] >= 5000 ? "5,000+" : rateRange[1].toLocaleString()}
                    </Typography>
                    <Slider
                      value={rateRange}
            ...
            <motion.div {...fadeUp(0.15)}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={3} flexWrap="wrap" gap={2}>
                <Box display="flex" alignItems="center" gap={1}>
                  <HandymanIcon sx={{ color: BRAND.orange }} />
                  <Typography variant="h6" fontWeight={700}>
                    {loading ? "Loading..." : `${meta.total} helper${meta.total !== 1 ? "s" : ""} found`}
                  </Typography>
                </Box>
              ...
              <>
                <Grid container spacing={3}>
                  {helpers.map((helper, i) => (
                    <Grid key={helper.id} size={{ xs: 12, sm: 6, xl: 4 }}>
                      <motion.div {...fadeUp(i * 0.05)}>
                        <PublicHelperCard helper={helper} />
                      </motion.div>
                    </Grid>
                  ))}
                </Grid>
